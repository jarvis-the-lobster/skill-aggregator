...
router.post('/', requireAuth, async (req, res) => {
  try {
    const { userType, goal, dailyTime } = req.body;
    ...
    await db.insert(
      `INSERT INTO user_onboarding (user_id, user_type, goal, daily_time)
       VALUES (?, ?, ?, ?)
       ON CONFLICT(user_id) DO UPDATE SET user_type=?, goal=?, daily_time=?, completed_at=CURRENT_TIMESTAMP`,
      [req.user.id, userType, goal, dailyTime, userType, goal, dailyTime]
    );
...
router.get('/', requireAuth, async (req, res) => {
  try {
    const rows = await db.query(
      'SELECT user_type, goal, daily_time, completed_at FROM user_onboarding WHERE user_id = ?',
      [req.user.id]
    );
    if (rows.length > 0) {
